...
export interface Court {
    city: string // Kota (e.g., "Pekanbaru")
    province: string // Provinsi (e.g., "Riau")
    court_name: string // Nama Pengadilan
    court_level: string // Tingkat Pengadilan (e.g., "Pengadilan Negeri", "Pengadilan Tinggi", "Mahkamah Agung")
  ...
  export interface CitedArticle {
    article: string // Pasal (e.g., "Pasal 2 Ayat (1)")
    law_name: string // Nama UU (e.g., "Undang-Undang Pemberantasan Tindak Pidana Korupsi")
    law_number: string | null // Nomor UU
    law_year: number | null // Tahun UU
  ...
  export interface Indictment {
    ...
    chronology: string // Kronologi Kejadian
    crime_location: string // Tempat Kejadian Perkara
    crime_period: {
      start_date: string | null
      end_date: string | null
      description: string // Deskripsi Waktu (e.g., "Tahun 2018")
    }
    cited_articles: CitedArticle[]
    defense_exception_status: string | null // Status Eksepsi
  }
  ...
  export interface StateLoss {
    currency: string // Mata Uang (e.g., "IDR")
    indicted_amount: number // Jumlah yang Didakwakan
    proven_amount: number | null // Jumlah yang Terbukti
    returned_amount: number | null // Jumlah yang Dikembalikan
    remaining_due: number | null // Sisa yang Harus Dibayar
    auditor: string | null // Auditor (e.g., "KAP Roni Pupung")
    audit_report_number: string | null
    audit_report_date: string | null
    perpetrators_proceeds: {
      // Hasil yang Diterima Pelaku
      name: string
      role: string
      amount: number | null
    }[]
  ...
  export interface LegalFacts {
    violations: string[] // Pelanggaran yang Dilakukan
    witness_testimonies: string[] // Keterangan Saksi
    documentary_evidence: string[] // Bukti Dokumen
    other_facts: string[] // Fakta Lainnya
    standard_procedures: string[] // Prosedur Standar yang Dilanggar
    financial_irregularities: string[] // Penyimpangan Keuangan
  ...
  export interface ProsecutionDemand {
    date: string | null
    content: string // Isi Tuntutan
    articles: CitedArticle[]
    prison_sentence_months: number | null
    prison_sentence_description: string | null
    fine_amount: number | null // Denda
    fine_subsidiary_confinement_months: number | null // Kurungan Pengganti Denda
    restitution_amount: number | null // Uang Pengganti
  ...
  export interface JudicialConsiderations {
    legal_element_considerations: string[] // Pertimbangan Unsur-unsur Pasal
    aggravating_factors: string[] // Hal yang Memberatkan
    mitigating_factors: string[] // Hal yang Meringankan
  }
  ...
  export interface CaseRecord {
    ...
    // Financial (for corruption cases)
    ...
    // Data quality
    extraction_confidence: number // 0-1 confidence score
  }
  ...
  export type CouncilMemberRole =
    | "penafsir_ketat" // Strict Constructionist
    | "rehabilitatif" // Humanist/Rehabilitative
    | "ahli_yurisprudensi" // Jurisprudence Historian
  
  export interface CouncilMember {
    id: CouncilMemberRole
    name: string // Display name (e.g., "Hakim A")
    title: string // Title (e.g., "Penafsir Ketat")
    description: string // Role description
    perspective: string // Legal perspective approach
    avatar_color: string // For UI display
  }
  ...
  export const COUNCIL_MEMBERS: Record<CouncilMemberRole, CouncilMember> = {
    penafsir_ketat: {
      ...
      perspective: "Menganalisis berdasarkan bunyi pasal secara tekstual dan yurisprudensi yang konsisten",
      avatar_color: "red",
    },
    rehabilitatif: {
      id: "rehabilitatif",
